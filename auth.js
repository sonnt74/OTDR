...
// Đã sửa lại hàm ẩn/hiện mật khẩu chuẩn xác theo thuộc tính type
...
### 📄 2. Tệp `admin.js` hoàn chỉnh (Phân quyền quản trị & Đổ dữ liệu form con)

Bạn hãy copy toàn bộ đoạn mã dưới đây và dán đè vào tệp **`admin.js`**[cite: 9]:

```javascript
// ==========================================================================
// TỆP ADMIN.JS - QUẢN TRỊ THEO PHÂN QUYỀN & ĐỔ DỮ LIỆU FORM CON
// ==========================================================================
...
// Lọc danh sách tài khoản theo phân quyền (Admin Đài / Trạm chỉ thấy đơn vị mình)
function getFilteredUsers() {
  ...
  if (role.includes('sys') || role === 'admin_sys') {
    return users; // Thấy tất cả
  } else if (role.includes('dai') || role === 'admin_dai') {
    return users.filter(u => String(u.id_dai) === String(user.id_dai || user.idDai));
  } else if (role.includes('tram') || role === 'admin_tram') {
    return users.filter(u => String(u.id_tram) === String(user.id_tram || user.idTram));
  }
  return users;
}
...
/** ĐỔ DỮ LIỆU ĐỘNG VÀO FORM THÊM/SỬA TÀI KHOẢN */
function chuanBiFormThemThanhVien(accToEdit) {
  ...
  if (accToEdit && accountInput) {
    ...
    accountInput.disabled = true; // Khóa tên tài khoản khi ở chế độ sửa
    if (passInput) passInput.value = '';
