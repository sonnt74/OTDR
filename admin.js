...
function loadAdminMasterData() {
  ...
  // --- TAB 1: RENDER DANH SÁCH TÀI KHOẢN ---
  ...
  // --- TAB 3: RENDER DANH SÁCH TRẠM VT ---
  ...
  tramFiltered.forEach(t => {
    ...
    var canEdit = (userRole === 'sys_admin') || (userRole === 'dai_admin' && String(t.id_dai) === myDaiId) || (userRole === 'tram_admin' && String(tId) === myTramId);
  ...
  // --- TAB 4: RENDER DANH SÁCH TUYẾN CÁP ---
  var tuyenHtml = '';
  rawTuyenList.forEach(t => {
    var tId = t.id_tuyen_cap || t.id_tuyen || t.id;
    var btnCmd = (userRole === 'sys_admin' || userRole === 'dai_admin') ? `<button class="btn-small" onclick="moFormSuaAux('tuyen_cap', '${tId}', '${t.ma_tuyencap || t.ten_tuyen}')">✏️ Sửa</button>` : `<span class="badge bg-secondary">Chỉ xem</span>`;
  ...
  // --- TAB 5: RENDER DANH SÁCH ĐOẠN TUYẾN ---
  var doanHtml = '';
...
function moFormSuaAux(tableType, recordId, currentName) {
  ...
  document.getElementById('auxModalTitle').innerText = "✏️ Cập Nhật Danh Mục";
  document.getElementById('auxFormFields').innerHTML = `
    <div class="form-group">
      <label>Tên mục chỉnh sửa:</label>
